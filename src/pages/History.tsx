  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // Transform API record to table format if needed
        const formatted = records.map((r: any) => ({
          id: r.id,
          date: r.date,
          time: r.inputs.time,
          weather: r.prediction.weather_condition,
          temp: r.prediction.temperature,
          traffic: r.prediction.vehicle_count,
          level: r.prediction.traffic_level,
          confidence: r.prediction.confidence || 85 // Fallback if missing
        }));
